...
- **规则文本、模型名、一条帖跑几遍 任一改，整批作废重判** —— 缓存里存着判的时候用的
  那版读法的指纹，对不上就整份丢掉（03§2.2）
...
def rule_fingerprint() -> str:
    """当前这版读帖规则的指纹。**规则文本、模型名、一条帖跑几遍，任一改指纹就变。**

    **模型名与跑几遍也都认**（02§11、§10.2）—— 换模型、换跑几遍，都是换了个读法，
    旧的判断不能接着用。只认规则文本的话，改了这两样会静悄悄地拿着上一版读法的
    结论往下跑。
    """
