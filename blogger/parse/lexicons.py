...
- **条件词** —— `has_cond` 判一句是不是条件句（§3.2）：`hints_of` 据此挑清单的「条件词」
  那一栏，`hints.suspects` 据此开复核的「条件句」那一栏。**它不删任何一行。**
...
**一条纪律。** 这几张表连同 `has_cond` 只喂提示词、清单与复核，**自己不删任何一行** ——
整个代码面按字面删行的判断只有 §10.2 强校验那一道。往后可以放心加宽：加错了最坏是把
模型带偏，不会误杀。改口径仍要走文档。
"""
...
# **`!` 的口径是「一定是条件句」，不是「像条件句」。** 它只喂两处 —— 清单的条件词那一栏
# （`hints_of`）与矛盾复核的条件句那一栏（`hints.suspects`），两处都不下判。所以宁可筛不
# 完（条件句没被认出的，实测有），也不许冤枉一条。下面几类是实测出来的「看着像条件、
# 其实不是」，逐类抠掉再匹配。
#
...
# **后果只到清单与复核为止** —— 多列一个词，模型据此自己判，不会误杀。
...
def has_cond(sentence: str) -> bool:
    ...
    只喂清单的条件词那一栏（`hints_of`）与矛盾复核的条件句那一栏（`hints.suspects`）——
    **不删任何一行**，也不下判。"""
    for d in COND_DECOY:
