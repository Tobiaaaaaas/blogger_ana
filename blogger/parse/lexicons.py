# -*- coding: utf-8 -*-
"""① 切与标 —— 六张标记词典。**从 02 的现成表逐行翻出来，不新增事实源。**

这一层只干两件事，都不涉及语义判断：

- **标记** —— 这一句里有没有时间词（`T`）／方向词（`D`）／对象词（`O`）／条件词（`!`）／
  负例词（`x`）／模棱词（`~`）。标记是给模型的**提示**，不是判决。
- **翻译** —— 模型指认的原话（「下周二」）翻成编码（`ntuesday`）。这是给模型减负：
  它指原文，这里翻表。

**词典永远有漏。** 模型答出表里没有的词，不是错 —— 那是缺口信号，由 `assemble` 记一笔。
所以这里宁缺毋滥：**拿不准的词一律不收**，让缺口暴露出来，好过收一个似是而非的。

方向词是开放类（「高位反复筑顶」这类说法追不全），时间词与对象词是闭集 —— 两者在
`assemble` 里的分量不同，见那里的说明。
"""
...
# 「8 月 20 日」这类具体日期 —— 落 `d:`，年份由 `assemble` 按发帖日补
...
def find_time_words(sentence: str) -> list[tuple[int, str, str]]:
    ...
    **同一位置取最长匹配**（「下周末」压过「下周」）。`d:` 与 `tN` 这两类由正则出，
    ...
    for m in _MD_RE.finditer(sentence):
        if free(m.start(), m.end()):
            # 年份留空，`assemble` 按发帖日补 —— 这里只标出「有个具体日期」
