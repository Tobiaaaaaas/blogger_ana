...
## 判定要点
1. **主结论句 doctrine（按行周期）**：被复核行对某周期（今天/本周…）的无条件方向，须与**该行所载周期**的明确主结论（方向+时段）一致；帖内**其它周期**另有独立无条件主结论时**不构成与本行的不一致**（一帖多周期各行独立，见教义节 8），**不得**把本行改写到别的周期。疑问/假设/条件/风险/点位预演不能覆盖主结论句。若整帖只是状态描述/复盘回顾/转述他人/仓位状态自述/无明确方向（如窄幅震荡、方向待定）而没有对 idx 所指指数的明确方向预测 → **drop**。
...
无任何其他文字。""".replace(
    "__DOCTRINE_REVIEW__", prompts.DOCTRINE_REVIEW)  # B3：复核教义条(6/7/8)同源，与报告 VERIFY 共享
