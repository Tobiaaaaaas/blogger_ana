...
【周期口径】spec 一律按**交易日**理解：t1=发帖后的第 1 个交易日（周末/假日发的"明天"仍记 t1，日历换算由系统做）；"后天/两日后"→t2；"N 天后/未来几天"这类自然日表述 → 填对应的 tN（系统按 N 个交易日计）。本周→week、下周→nweek、**"下周一"→ 该日即发帖后首个交易日（周五/周六/周日发帖时通常如此）则 t1、否则 nweek_first**（2026-09-10 编码纠正：t1 的定义就是"发帖后第 1 个交易日"，"下周一"落在那一格时二者同义）、**「本周」已走完时发帖说"本周"（典型：周六/周日发帖）：该行不得产 `week` —— 回顾句不产行、前瞻句编 `nweek`；本周尚未走完（周一~周五发帖、含周中假日但本周之后仍有交易日）仍正常 `week`（完整判据见下方 `DOCTRINE_NO_DIRECTION` 的对应条目；2026-09-10 周口径修正）**、月底前→month、下月→nmonth；今年/下半年/年度/长期/未来几个月 → long（unscored）。**未来具体日期（"X月X日"）直接落 `d:YYYY-MM-DD`（2026-09-17 放开）**：历法与交易日换算由系统做，**不要折算成 tN**；带 本周/下周/月底 等**周期词** → 跟词走 week/nweek（"下周一"按上述分叉落 t1 或 nweek_first）/month/nmonth；日期明显远期（跨下月以后/跨年/"某日见顶见底"类远期节点）→ long（unscored）。
...
## 第一步：判定主结论句
先找到帖子的**主结论句**——形如「周三：大盘探底回升，我看涨」「明天：只卖不买」「下周一我看跌」等**带明确方向**的句子。主结论句是全帖最高优先级的预测：**任何条件句、风险提示、走势分类、点位预演都不能替代或覆盖主结论句**。若整帖唯一可提取内容就是条件句/先A后B形态（守/破/分水岭/冲高回落…）、并无带方向的独立无条件主结论 → 该帖无主结论（按第 2 条 drop，不把分支偏好当方向）。**一帖可有多个周期的独立无条件主结论**（"今天市场会大涨"＋"综合看这周市场启动的判断不变"、或"今天看多＋本周看空"）：每行只按**该行所载周期**的那句无条件主结论判 keep/fix/drop，与帖内其它周期的无条件句**互不构成不一致**——不得把较远周期行 cross-fix 到近端"主结论"周期的方向/周期。若提取信号与**该行所载周期**的主结论句方向/周期不一致 → 必须 action=fix，改回该周期无条件主结论句的方向/周期；某行所在周期内只有条件式/形态句（守/破/分水岭/冲高回落…含"这周高开低走""今天冲高回落"有周期词的形态句）、无该周期的独立无条件方向 → 按本行 drop 或 fix 到该周期无条件句，只影响本行、不影响其它周期行（同复核教义节 8）。

## 教义（单一同源，与推送复核同一文本，2026-09-09 B3；编号沿用推送复核判定要点 6/7/8）
__DOCTRINE_REVIEW__
...
2. **可打分性**：只有形态描述（震荡/筑底/洗盘/冲高回落/支撑位）而无明确方向态度 → drop（"震荡下跌/震荡上涨"除外，属明确方向）；**模棱两可/方向不明（"可能涨也可能跌""边走边看""不好说""即将变盘"）→ drop（连单列都不记）；纯复盘/对过去的分析（回顾行情、评价已发生走势、事后总结）→ drop；状态描述（"已经进入调整阶段""顶背离已出现"）→ drop**；仓位状态自述 → drop。条件式/形态主句的硬凑 d 与形态句后的净方向句 → 一律按教义节 6/7 处置（硬凑 d 且帖无独立无条件主结论 → drop；有 → fix 到该部分；净方向句属主结论 → 按第一步 keep/fix、不 drop）；多周期各行 → 按教义节 8 独立评审（某行所在周期无独立无条件方向才 drop/fix 该行，不 cross-fix 其它周期行）。
3. **盘中/盘前"今天"**："今天/今日/下午/午后/尾盘"预测 → spec=today、cat=scored（**无论盘前/盘中/盘后/非交易日发布都保持**，是否过时由打分引擎自动判定，审查阶段不做无效判断）。
4. **补加**：原文有比已提取信号**更明确或被漏掉**的预测结论 → 放入 add。典型遗漏：① 同一帖子里还有**第二个观点**（不同方向或不同周期，如"短期看空、长期看多"只提取了短期看空 → add 长期看多那条：long/unscored；同一帖 今天/本周 各一行的"近端已抽、远端漏抽"，如只抽 today 漏 week → 同样属漏，按教义节 8 add 远端那条）；② 应属 t5（无周期有方向）或 long/unscored 而未被提取。
