...
- **开档先备料行情**（02§2.2 / 06§5.3）：把七个主指数的 30 分钟线补到本档时刻，**备不到本档不推**
  —— 放在抓帖之前，不白抓 46 位博主、不白调一次标注。行情取值与报告链、打分器同源（opinion.ref_price）。
...
def _process_board(key, anchored, counts, posters, mkt_text, now, date_str, hm):
    ...
    for name, r in (anchored or {}).items():
        ep = r.get("endpoint")
        log.info("[%s] 上卡 %s · 帖 %s · spec=%s · %s · 终点 %s · anchor %s",
                 key, name, r.get("post_id"), r.get("spec"), r.get("horizon"),
...
def _run(args):
    ...
    # 0) 备料行情到本档时刻（02§2.2 / 06§5.3）—— 已到本档时刻就跳过不重抓、没到补一次、
    #    补完还不到则本档不推。解析要用发帖时刻那根 30 分钟线。放在抓帖之前：备不到就整档
    #    不做，不白抓 46 位博主、不白调一次标注。
    if not market.prepare_history(now):
        log.error("行情备不到本档时刻 %s —— 本档不推（02§2.2）", hm)
        return 0
