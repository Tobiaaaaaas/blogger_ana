...
v22（2026-09-17）：回看窗口**两板块统一 3 个交易日**，并加**起点下限** WINDOW_FLOOR
  （用户指示）——与仓库根新栈同一口径：
  - 窗口起点 = max(前 3 个交易日的 00:00, WINDOW_FLOOR)。下限设 2026-09-17 02:00（美联储
    决议落地）：落地之前的观点背景已经换过一轮，一律不进窗口。下限压过交易日起点时，
    卡面覆盖行改写成起点时刻（run_briefing._window_txt）。它早晚失效——前 3 个交易日的
    00:00 一旦晚过它就不再起作用。
v15（2026-09-07）：推送节奏**两板块统一**（用户指示）：
  - 超短板块（21，2026-09-07 换名单）：扫 WINDOW_TRADING_DAYS["short"] → 起至现在的
    今天/明天 表态，只显示 目标日∈{卡片日,下一交易日} 者。
  - 波段板块（29）：扫 WINDOW_TRADING_DAYS["swing"] → 起至现在的
...
名单口径：
  - 超短板块（21，2026-09-07 更新）：超短档（0-1 交易日）质量筛选——胜率>50% 且 平均分>0.1 且
    ...
    展示顺序。旧 17 人名单（源自 reports/top20_值得关注博主.md 超短榜，文件已归档
    archive/20260909-manual-reports/）已注释保留在 PANEL_SHORT 下方，备日后参考。
  - 波段板块（29，2026-09-16 更新）：波段档（信号日→验证终点≥2 交易日）三条件全严格达标者
    27 人（均分>0.2 且 夏普>0.1 且 胜率>50%，数据截止 2026-09-04）按均分降序 + 原委员会保留
    刘海娃娃/顺应周期 殿后；剔除 红红火火的老牛哥、微风3241；再剔除 一只小小牛（账号已注销）；
    股评老陈 2026-09-16 换号，按其显示名记作 知心朋友股评老陈。完整筛选口径见
    reports/comparison_direction.md 与当期《波段达标名单》。顺序即板块内展示顺序。
"""
from datetime import datetime

# ── 超短板块（21）：只看 今天/明天 方向观点 ──
# 旧名单（17 人 · 源自 reports/top20_值得关注博主.md 超短榜，文件已归档 archive/20260909-manual-reports/ · 2026-09-07 下线，注释保留备查）：
...
# ── 波段板块（29）：只看 波段(近日/本周/下周/更长) 方向观点 ──
PANEL_SWING = [
    "股往金来oo", "家有高中生2", "故乡的云ZYH", "子房论市",
    "选对时机买对股", "香满衣", "知行合一", "智由智哉", "知心朋友股评老陈",
...
# ── 展示窗口（v14 交易日口径；v22 两板块统一 3 日 + 起点下限）──
# 交易日起点 = 前 N 个交易日的 00:00（n_trading_days_ago）；窗口内容面 = 前 N 个交易日的
# 全天帖 + 今日盘中至 now；now 非交易日按最近交易日取参考日。由此周一早晨的窗口天然含
# 上周五帖（消除 v13 自然日口径的"周一漏上周五帖"取舍）。
# 起点最终取 max(交易日起点, WINDOW_FLOOR) —— 下限是给"盘外大事落地"留的，落地之前的
# 观点背景已经换过一轮。空串＝不限。
WINDOW_TRADING_DAYS = {"short": 3, "swing": 3}
WINDOW_FLOOR = "2026-09-17 02:00"     # 北京时间；美联储决议落地。早晚会失效，届时删掉即可
