...
【周期口径】spec 一律按**交易日**理解：t1=发帖后的第 1 个交易日（周末/假日发的"明天"仍记 t1，日历换算由系统做）；"后天/两日后"→t2；"N 天后/未来几天"这类自然日表述 → 填对应的 tN（系统按 N 个交易日计）。本周→week、下周→nweek（特指下周一才用 nweek_first）、月底前→month、下月→nmonth；今年/下半年/年度/长期/未来几个月 → long（unscored）。只填 spec 档位，不要写具体日期。**未来具体日期（"X月X日"）同样只落档位、不写 d:YYYY-MM-DD**：带 本周/下周/月底 等词 → 就近 week/nweek（下周一才 nweek_first）/month/nmonth，有词跟词；否则按该日期距发帖日的**自然日差**落 tN（如 8/10 发帖说"8月20日"→ t10），自然日差 >30 或明显远期（跨下月以后/跨年/"某日见顶见底"类远期节点）→ long（unscored）。
